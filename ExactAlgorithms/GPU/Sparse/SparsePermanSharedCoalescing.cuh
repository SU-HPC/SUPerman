...
#ifndef SUPERMAN_REBORN_SPARSEPERMANSHAREDCOALESCING_CUH
#define SUPERMAN_REBORN_SPARSEPERMANSHAREDCOALESCING_CUH
...
class SparsePermanSharedCoalescing: public Permanent<C, S>
{
public:
    SparsePermanSharedCoalescing(Matrix<S>* matrix, Settings settings)
...
template<class C, class S>
double SparsePermanSharedCoalescing<C, S>::permanentFunction()
{
    ...
    cudaOccupancyMaxPotentialBlockSizeVariableSMem(&grid_dim,
                                                   &block_dim,
                                                   &Definitions::kernel_xshared_coalescing_sparse<C,S>,
    ...
    Definitions::kernel_xshared_coalescing_sparse<C,S><<<grid_dim , block_dim , size>>> (d_cptrs, d_rows, d_cvals, d_x, d_p, nov);
    cudaDeviceSynchronize();
...
#endif //SUPERMAN_REBORN_SPARSEPERMANSHAREDCOALESCING_CUH
