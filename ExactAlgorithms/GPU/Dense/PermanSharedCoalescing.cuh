...
#ifndef SUPERMAN_REBORN_PERMANSHAREDCOALESCING_CUH
#define SUPERMAN_REBORN_PERMANSHAREDCOALESCING_CUH
...
class PermanSharedCoalescing: public Permanent<C, S>
{
public:
    PermanSharedCoalescing(Matrix<S>* matrix, Settings settings)
...
template<class C, class S>
double PermanSharedCoalescing<C, S>::permanentFunction()
{
    ...
    int sync_gray = this->m_Settings.synchronizedGray;
    //Pack flags//
    ...
    Definitions::kernel_xshared_coalescing<C,S><<<grid_dim , block_dim , size>>> (d_mat_t, d_x, d_p, nov);
    cudaDeviceSynchronize();
...
#endif //SUPERMAN_REBORN_PERMANSHAREDCOALESCING_CUH
