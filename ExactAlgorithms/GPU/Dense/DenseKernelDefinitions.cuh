...
namespace DenseDefinitions
{
    ...
    template <class C, class S>
    __global__ void xRegisterMGlobalMatSpecificCompilation(S* mat,
                                     C* x,
                                     C* p,
                                     int nov,
                                     long long start,
                                     ...
                                     long long chunkSize)
    {
        ...
        #define REG(reg, number) C reg = x[number];
            SPECIFIC
        #undef REG
        ...
        for (int j = 0; j < (NOV - 1); ++j)
        {
            if ((gray >> j) & 1LL) // was jth column included?
            {
                #define REG(reg, number) reg += mat[j * NOV + number];
                    SPECIFIC
                #undef REG
            }
        }
        ...
        for (long long i = myStart; i < myEnd; ++i)
        {
            ...
            C product = 1;
            #define REG(reg, number) reg += added * mat[columnChanged * NOV + number]; product *= reg;
                SPECIFIC
            #undef REG
